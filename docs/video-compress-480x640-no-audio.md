...
```bash
python3 tools/compress_onboarding_videos.py "/path/to/video-folder" --size 480x640 --crf 26 --open
```
...
flowchart TD
  ...
  D --> E["选择压缩强度"]
  E --> F["缩放并裁剪为目标尺寸"]
  F --> G["libx264 / selected CRF / preset slow 编码"]
  G --> H["移除音轨"]
  H --> I["解码校验"]
  I --> J["生成首帧海报"]
  J --> K["写入 final_report.tsv / summary.md"]
  K --> L["打开输出目录"]
```
